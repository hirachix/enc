...
-- Tambahkan fungsi untuk set BypassDelay secara langsung
local function SetBypassDelay(value)
    local number = tonumber(value)
    if number and number >= 0 then
        BypassDelay = number
        WindUI:Notify({
            Title = "Bypass Delay",
            Content = string.format("Set to: %.2fs", BypassDelay),
            Duration = 2
        })
        return true
    else
        WindUI:Notify({
            Title = "Error",
            Content = "Invalid input! Must be a number >= 0",
            Duration = 2
        })
        return false
    end
...
-- Input untuk Bypass Delay (TAMBAHAN BARU)
MainTab:Input({
    Title = "Bypass Delay",
    Icon = "lucide:clock",
    Desc = "Waktu tunggu utama sebelum bite (detik)",
    Value = tostring(BypassDelay),
    Callback = function(value)
        SetBypassDelay(value)
    end
})
