-- ============================================================================
-- HIRAKO FISH IT - FINAL UPGRADED VERSION (FULL INTEGRATED + WEBHOOK)
-- ============================================================================
...
-- [ADDED] Discord Webhook Integration V3
...
-- ============================================================================
-- FISH WEBHOOK SYSTEM V3 (INTEGRATED)
-- ============================================================================
...
local function buildThumbnailCacheAsync(fishData)
    log("Starting background thumbnail cache...")
    ...
    log("Will fetch " .. #assetIds .. " thumbnails in background...")
    ...
    state.thumbnailThread = task.spawn(function()
        ...
        for i = 1, #assetIds, batchSize do
            if isCancelled then
                log("Thumbnail loading cancelled")
                break
            end
            ...
            log(string.format("Thumbnail batch %d/%d complete (%d loaded)", 
        ...
        if not isCancelled then
            ...
            log("Background thumbnail cache COMPLETE: " .. #assetIds .. " entries")
        end
...
local function buildEmbed(info)
    ...
    local embed = {
        ...
        footer = {text = "HIRAKO | Fish-It Notifier"},
        fields = {
...
local function queueFish(info)
    if not shouldSendFish(info) then
        log("Fish tier not selected: " .. (info.name or "Unknown"))
        return
    end
    
    local sig = createSig(info)
    if isDuplicate(sig) then
        log("Duplicate fish: " .. (info.name or "Unknown"))
        return
    end
    ...
    log("Queued fish: " .. (info.name or "Unknown"))
end
...
local function connectEvents()
    ...
    log("Waiting for " .. CFG.TARGET_EVENT .. "...")
end
...
function FishWebhookV3:Init()
    ...
    log("Thumbnail cache: Loading in background...")
...
function FishWebhookV3:TestWebhook(msg)
    ...
    return sendWebhook({
        username = "HIRAKO Notifier",
        content = msg or "🎣 Test from HIRAKO Fish-It v2.0"
    })
end
...
local Config = {
    ...
    WebhookUrl = "",
    WebhookSelectedTiers = {"SECRET", "MYTHIC", "LEGENDARY"},
}
...
local function SaveConfig()
    local configData = {
        Config = {
            ...
            WebhookSelectedTiers = Config.WebhookSelectedTiers,
        },
...
TelegramTab:Toggle({
    ...
    Callback = function(state)
        ...
        if state then
            ...
            local success = FishWebhookV3:Init()
            if success then
                FishWebhookV3:Start({
                    webhookUrl = Config.WebhookUrl,
                    selectedTiers = Config.WebhookSelectedTiers
                })
                notify("Discord Webhook", "Webhook system started!", 3)
            else
                notify("Webhook Error", "Failed to initialize webhook system", 3)
                Config.WebhookEnabled = false
            end
        else
            FishWebhookV3:Stop()
            notify("Discord Webhook", "Webhook system stopped", 3)
        end
    end
})
...
TelegramTab:Input({
    ...
    Callback = function(val)
        ...
        FishWebhookV3:SetWebhookUrl(val)
        SaveConfig()
        notify("Webhook URL", "URL saved!", 2)
    end
})

-- Tier selection untuk webhook
local webhookTiers = {"COMMON", "UNCOMMON", "RARE", "EPIC", "LEGENDARY", "MYTHIC", "SECRET"}

for _, tier in ipairs(webhookTiers) do
    TelegramTab:Toggle({
        Title = "Webhook: " .. tier,
        Icon = "lucide:fish",
        Desc = "Send " .. tier .. " fish to Discord",
        Value = table.find(Config.WebhookSelectedTiers, tier) ~= nil,
        Callback = function(val)
            if val then
                if not table.find(Config.WebhookSelectedTiers, tier) then
                    table.insert(Config.WebhookSelectedTiers, tier)
                end
            else
                local index = table.find(Config.WebhookSelectedTiers, tier)
                if index then
                    table.remove(Config.WebhookSelectedTiers, index)
                end
            end
            
            FishWebhookV3:SetSelectedTiers(Config.WebhookSelectedTiers)
            SaveConfig()
        end
    })
end
...
TelegramTab:Button({
    ...
    Desc = "Send test message to Discord",
    Callback = function()
        if Config.WebhookUrl == "" then
            notify("Error", "Enter Webhook URL first!", 2)
            return
        end
        
        local success = FishWebhookV3:TestWebhook("🎣 **HIRAKO FISH IT** - Webhook test successful!")
        notify("Webhook Test", success and "Test sent!" or "Test failed!", 3)
    end
})
...
TelegramTab:Button({
    ...
    Desc = "Check webhook system status",
    Callback = function()
        local status = FishWebhookV3:GetStatus()
        local msg = string.format(
            "Webhook Status:\nRunning: %s\nURL: %s\nSelected Tiers: %d\nQueue: %d\nCache: %d fish",
            tostring(status.running),
            status.webhookUrl,
            #Config.WebhookSelectedTiers,
            status.queueSize,
            status.fishCacheSize
        )
        notify("Webhook Status", msg, 5)
    end
})
...
LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    if Config.WebhookEnabled and Config.WebhookUrl ~= "" then
        ...
        if success then
            ...
            print("[WEBHOOK] Discord webhook system reinitialized after respawn")
        end
    end
...
local function Initialize()
    ...
    if Config.WebhookEnabled and Config.WebhookUrl ~= "" then
        ...
        if success then
            ...
            print("[WEBHOOK] Discord webhook system initialized")
        else
            warn("[WEBHOOK] Failed to initialize webhook system")
            Config.WebhookEnabled = false
        end
    end
...
-- Cleanup webhook on script end
game:GetService("Players").PlayerRemoving:Connect(function(player)
    if player == LocalPlayer then
        FishWebhookV3:Cleanup()
    end
end)